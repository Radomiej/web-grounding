...
1. Dodaj kolumnę `ID` albo dostępność do tabeli.
2. Zmień sortowanie i wyjaśnij `ORDER BY`.
3. Przetestuj pustą tabelę, błąd zapytania i zatrzymany MySQL.
4. Sprawdź, czy wartości z bazy są escapowane przez `htmlspecialchars`.

## Kryteria zaliczenia

- Skrypt otwiera połączenie, wykonuje `SELECT`, przechodzi po rekordach i zamyka zasoby.
- Widoki błędu oraz pustej tabeli są opisane i nie kończą się ostrzeżeniem PHP.
- Dane z bazy trafiają do HTML dopiero po `htmlspecialchars`.
