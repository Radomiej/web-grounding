# Ćwiczenie 401 — PHP w HTML

Uruchom `index.php` przez Apache/XAMPP. Plik najpierw przygotowuje dane w PHP, a później przechodzi do HTML. To początek serii `4xx`; wybierz ją wtedy, gdy arkusz wymaga PHP i bazy.
...
1. Zmień imię i dodaj temat do tablicy.
2. Zmodyfikuj próg godziny i sprawdź obie gałęzie warunku.
3. Porównaj `echo` ze skrótem `<?= ... ?>`.
4. Escapuj własną wartość i sprawdź znaki `<`, `>` oraz `&`.

## Kryteria zaliczenia

- Strona uruchamia się w lokalnym interpreterze PHP i pokazuje tytuł oraz listę tematów.
- Uczeń potrafi wskazać zmienną, tablicę, warunek, pętlę `foreach` i bezpieczne escapowanie.
- Zmiana danych wejściowych widocznie zmienia wynik bez błędów składni.
